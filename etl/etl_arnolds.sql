/* ============================================================
   ...
   v6 — Added Step 2b: dim_teachers
        · Copies teaching staff from arnolds1.teachers into
          student_analytics.dim_teachers so the login page
...
/* ============================================================
   ...
   Copies all active staff from arnolds1.teachers into
   student_analytics.dim_teachers.
   ...
   directly to arnolds1.
   ============================================================ */

CREATE TABLE IF NOT EXISTS student_analytics.dim_teachers (
...
TRUNCATE TABLE student_analytics.dim_teachers;

INSERT INTO student_analytics.dim_teachers
...
INSERT INTO student_analytics.dim_subject (subject_id, subject_name, subject_type)
SELECT sm_id, name, subject_type
FROM arnolds1.subject_master
WHERE name NOT IN (
  ...
  AND NOT EXISTS (
    SELECT 1 FROM student_analytics.dim_subject ds WHERE ds.subject_id = sm_id
  )
ORDER BY sm_id;
...
DELETE FROM student_analytics.dim_subject WHERE subject_id = 24;
INSERT INTO student_analytics.dim_subject (subject_id, subject_name, subject_type)
VALUES (24, 'Science', 'Scholastic');

SET @_idx = (
    SELECT COUNT(1) FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = 'student_analytics'
      AND TABLE_NAME   = 'dim_subject'
      AND INDEX_NAME   = 'uq_dim_subject_id'
);
SET @_sql = IF(@_idx > 0,
    'DROP INDEX uq_dim_subject_id ON student_analytics.dim_subject',
    'SELECT 1'
);
PREPARE _s FROM @_sql; EXECUTE _s; DEALLOCATE PREPARE _s;
ALTER TABLE student_analytics.dim_subject ADD UNIQUE INDEX uq_dim_subject_id (subject_id);
...
CREATE TABLE IF NOT EXISTS student_analytics.dim_class_subject_map (
...
INSERT INTO student_analytics.dim_class_subject_map (class_name, subject_id, academic_yr)
...
FROM (
    SELECT DISTINCT
        ...
        sub.academic_yr
    FROM arnolds1.subject sub
    JOIN arnolds1.class c ON c.class_id = sub.class_id AND c.academic_yr = sub.academic_yr
    JOIN arnolds1.subject_master sm_src ON sm_src.sm_id = sub.sm_id
...
FROM (
    SELECT
        ...
        1 AS is_entered
    FROM arnolds1.student_marks_components smc
    JOIN arnolds1.student st ON st.student_id = smc.student_id AND st.academic_yr = smc.academic_yr
    JOIN arnolds1.class c ON c.class_id = st.class_id AND c.academic_yr = st.academic_yr
    JOIN arnolds1.subject sub ON sub.sm_id = smc.subject_id AND sub.academic_yr = smc.academic_yr AND sub.class_id = st.class_id
    WHERE smc.academic_yr IN (SELECT academic_yr FROM new_years)
...
JOIN arnolds1.homework_comments hc ON h.homework_id = hc.homework_id
...
INSERT IGNORE INTO student_analytics.dim_class (class_id, class_name)
SELECT DISTINCT c.class_id, c.name FROM arnolds1.class c
WHERE c.class_id IN (SELECT DISTINCT s.class_id FROM arnolds1.student s
    WHERE s.academic_yr BETWEEN '2019-2020' AND '2025-2026' AND s.IsDelete = 'N')
  AND NOT EXISTS (SELECT 1 FROM student_analytics.dim_class dc WHERE dc.class_id = c.class_id);

INSERT IGNORE INTO student_analytics.dim_section (section_id, section_name)
SELECT DISTINCT sec.section_id, sec.name FROM arnolds1.section sec
WHERE sec.section_id IN (SELECT DISTINCT s.section_id FROM arnolds1.student s
    WHERE s.academic_yr BETWEEN '2019-2020' AND '2025-2026' AND s.IsDelete = 'N')
  AND NOT EXISTS (SELECT 1 FROM student_analytics.dim_section ds WHERE ds.section_id = sec.section_id);


-- FIX B: Re-insert any students still missing
INSERT INTO student_analytics.student_clean (student_id, academic_yr, reg_no, class_id, section_id)
SELECT s.student_id, s.academic_yr, s.reg_no, s.class_id, s.section_id
FROM arnolds1.student s
WHERE s.academic_yr BETWEEN '2019-2020' AND '2025-2026' AND s.IsDelete = 'N'
  AND NOT EXISTS (SELECT 1 FROM student_analytics.student_clean sc
    WHERE sc.student_id = s.student_id AND sc.academic_yr = s.academic_yr);

INSERT INTO student_analytics.dim_student_demographics
    (student_id, academic_yr, student_name, gender, dob,
...
FROM arnolds1.student s
WHERE s.academic_yr BETWEEN '2019-2020' AND '2025-2026' AND s.IsDelete = 'N'
  AND NOT EXISTS (SELECT 1 FROM student_analytics.dim_student_demographics d
    WHERE d.student_id = s.student_id AND d.academic_yr = s.academic_yr);

INSERT INTO student_analytics.student_master_profile
    (student_id, academic_yr, reg_no, class_id, section_id,
     avg_percent, written_avg, oral_avg, exams_taken,
     attendance_percentage, achievements, homework_assigned_count)
SELECT s.student_id, s.academic_yr, s.reg_no, s.class_id, s.section_id,
...
FROM student_analytics.student_clean s
LEFT JOIN student_analytics.fact_academics           fa   ON fa.student_id   = s.student_id AND fa.academic_yr   = s.academic_yr
LEFT JOIN student_analytics.fact_attendance          fatt ON fatt.student_id = s.student_id AND fatt.academic_yr = s.academic_yr
LEFT JOIN student_analytics.fact_achievements        fach ON fach.student_id = s.student_id AND fach.academic_yr = s.academic_yr
LEFT JOIN student_analytics.fact_homework_engagement fhw  ON fhw.student_id  = s.student_id AND fhw.academic_yr  = s.academic_yr
WHERE s.academic_yr BETWEEN '2019-2020' AND '2025-2026'
  AND NOT EXISTS (SELECT 1 FROM student_analytics.student_master_profile sm
    WHERE sm.student_id = s.student_id AND sm.academic_yr = s.academic_yr);


-- FIX C: Re-insert any marks still missing
INSERT INTO student_analytics.fact_student_subject_performance
    (student_id, academic_yr, subject_id, avg_percent, written_avg, oral_avg, exams_taken)
...
FROM arnolds1.student_marks_components smc
JOIN arnolds1.student st ON st.student_id = smc.student_id AND st.academic_yr = smc.academic_yr AND st.IsDelete = 'N'
JOIN arnolds1.class c ON c.class_id = st.class_id AND c.academic_yr = st.academic_yr
JOIN arnolds1.subject sub ON sub.sm_id = smc.subject_id AND sub.academic_yr = smc.academic_yr AND sub.class_id = st.class_id
WHERE smc.academic_yr BETWEEN '2019-2020' AND '2025-2026' AND smc.is_present = 'Y'
  AND NOT EXISTS (
    SELECT 1 FROM student_analytics.fact_student_subject_performance fsp
    WHERE fsp.student_id = smc.student_id AND fsp.academic_yr = smc.academic_yr
      AND fsp.subject_id = CASE WHEN c.name IN ('6','7','8','9','10') AND smc.subject_id IN (15,16,17) THEN 24 ELSE smc.subject_id END
  )
GROUP BY smc.student_id, smc.academic_yr,
...
INSERT IGNORE INTO student_analytics.dim_parent (parent_id, father_name, mother_name)
SELECT parent_id, father_name, mother_name FROM arnolds1.parent
WHERE NOT EXISTS (SELECT 1 FROM student_analytics.dim_parent dp WHERE dp.parent_id = arnolds1.parent.parent_id);


-- FIX E: Ghost student preview (review before uncommenting deletes)
SELECT sm.student_id, sm.academic_yr, d.student_name
FROM student_analytics.student_master_profile sm
LEFT JOIN student_analytics.dim_student_demographics d ON d.student_id = sm.student_id AND d.academic_yr = sm.academic_yr
LEFT JOIN arnolds1.student s ON s.student_id = sm.student_id AND s.academic_yr = sm.academic_yr
WHERE sm.academic_yr BETWEEN '2019-2020' AND '2025-2026'
  AND (s.student_id IS NULL OR s.IsDelete = 'Y');

/*
DELETE sm FROM student_analytics.student_master_profile sm
LEFT JOIN arnolds1.student s ON s.student_id = sm.student_id AND s.academic_yr = sm.academic_yr
WHERE sm.academic_yr BETWEEN '2019-2020' AND '2025-2026' AND (s.student_id IS NULL OR s.IsDelete = 'Y');

DELETE sbs FROM student_analytics.student_behavior_signals sbs
LEFT JOIN arnolds1.student s ON s.student_id = sbs.student_id AND s.academic_yr = sbs.academic_yr
...
LEFT JOIN (SELECT academic_yr, COUNT(DISTINCT student_id) AS in_master FROM student_analytics.student_master_profile GROUP BY academic_yr) smp ON smp.academic_yr = src.academic_yr
LEFT JOIN (SELECT academic_yr, COUNT(DISTINCT student_id) AS in_attendance FROM student_analytics.fact_attendance GROUP BY academic_yr) att ON att.academic_yr = src.academic_yr
LEFT JOIN (SELECT academic_yr, COUNT(DISTINCT student_id) AS have_marks FROM student_analytics.fact_student_subject_performance GROUP BY academic_yr) mrk ON mrk.academic_yr = src.academic_yr
...
FROM student_analytics.dim_teachers;
...
SELECT subject_id, subject_name FROM student_analytics.dim_subject
...
FROM arnolds1.student_marks_components smc
JOIN arnolds1.marks_headings mh ON mh.marks_headings_id = smc.marks_headings_id
